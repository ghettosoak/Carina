...
$pulledproj = mysql_query('select name, client, text, contrib, allprojpromoted from project where id = "'.$whichproj.'"');
...
while ($theproj = mysql_fetch_array($pulledproj)){
	$thisproj['details'] = array(
		'name' => $theproj['name'], 
		'client' => $theproj['client'], 
		'text' => $theproj['text'],
		'contrib' => $theproj['contrib'],
		'allprojpromoted' => $theproj['allprojpromoted']
	);
}
